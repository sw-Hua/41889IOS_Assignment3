...
//  Example-Booking
//
//  Created by Songwen Hua on 2022/5/15.
//
...
//                })
            ...
            Text("Maximum Select 1 Table").font(.title3).foregroundColor(.white).fontWeight(.bold)
            HStack(spacing: 30){
                ...
                }).alert(isPresented: $alertIsTure, content: {
                    Alert(title: Text("Dear Customer, you only can book 1 table"), message: Text("Due to tight bookings"), dismissButton: .default(Text("confirm")))
                })
            ...
            HStack{
                ...
                DatePicker("  ", selection: $date, in: Date.now..., displayedComponents: .date).background(.white).cornerRadius(10).font(.title3).frame(width: 130).shadow(color: Color.white, radius: 5, x: 0, y: 0).foregroundColor(.gray)
            ...
            HStack(spacing: 30){
                ...
                if(selectedSelects.count == 0){
                    ...
                    }).background(.gray.opacity(0.8)).cornerRadius(20).padding().alert(isPresented: $alertIsTure2, content: {
                        Alert(title: Text("Dear Customer, you should minimum book 1 table"), message: Text("minimum book 1 table"), dismissButton: .default(Text("Select One")))
                    })
